...
class unet(nn.Module):
    def __init__(self, inChannels, outChannels):
        ...
        self.down_convolution_1 = double_convolution(inChannels, 64)
        ...
        # output => `out_channels`
        self.out = nn.Conv2d(
            in_channels=64, out_channels=outChannels,
            kernel_size=1
        )
...
if __name__ == '__main__':
    net = unet(inChannels=3, outChannels=3)
    x = Variable(torch.FloatTensor(16, 3, 240, 240))
    y = net(x)
    print(y.data.shape) #Output shape for this input --> torch.Size([16, 3, 240, 240])